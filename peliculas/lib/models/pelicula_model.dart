//tenemos dos manera de generar este archivo,usar la extension pasteJsonAsCode que por medio del json de
//la respuesta, va a generar el modelo
//o, lo hacemos a mano

//esta la creo despues de la clase Pelicula
class Peliculas{
  List<Pelicula> items = [];

  //constructor
  Peliculas({required this.items});

  //constructor nuevo
  factory Peliculas.fromJsonList(Map<String, dynamic> jsonList){
   
    var listMovies = jsonList['results'] as List;

    List<Pelicula>? peliculasList = listMovies.map((item) =>Pelicula.fromJsonMap(item)).toList();

    return Peliculas(
      items: peliculasList
    );
    
    /* for(var item in jsonList){
      final pelicula = Pelicula.fromJsonMap(item); //recibo un mapa de los tags de pelicula y la hago un objeto pelicula

      items.add(pelicula); */
    
  } 
}


class Pelicula {
  final bool adult;
  final String backdropPath;
  final List<int> genreIds;
  final int id;
  final String originalLanguage;
  final String originalTitle;
  final String overview;
  final double popularity;
  final String posterPath;
  final String releaseDate;
  final String title;
  final double voteAverage;
  final int voteCount;

  Pelicula({
    required this.adult,
    required this.backdropPath,
    required this.genreIds,
    required this.id,
    required this.originalLanguage,
    required this.originalTitle,
    required this.overview,
    required this.popularity,
    required this.posterPath,
    required this.releaseDate,
    required this.title,
    required this.voteAverage,
    required this.voteCount,
  });


  //nuevo constructor que va a ser una funcion a usar cuando quiera generar una instancia de pelicula
  //que viene en un formato JSON

  factory Pelicula.fromJsonMap(Map<String, dynamic> json) {
    return Pelicula( 
      adult               : json['adult'] as bool,
      backdropPath        : json['backdrop_path'] as String,
      genreIds            : json['genre_ids'].cast<int>(),//para que sean esteros
      id                  : json['id'] as int,
      originalLanguage    : json['original_language'] as String,
      originalTitle       : json['original_title'] as String,
      overview            : json['overview'] as String,
      popularity          : json['popularity'] / 1 as double,
      posterPath          : json['poster_path'] as String,
      releaseDate         : json['release_date'] as String,
      title               : json['title'] as String,
      voteAverage         : json['vote_average'] / 1 as double,//para que lo tome como double cuando venga cerrado[ 5.0 => se transforma en 5 (entero)]
      voteCount           : json['vote_count'] as int
    );
  }

  getPosterImg(){
    // ignore: unnecessary_null_comparison
    if(posterPath == null){
      return 'data:image/png;base64,';
    }
    return 'https://image.tmdb.org/t/p/w500$posterPath';
  }
}

